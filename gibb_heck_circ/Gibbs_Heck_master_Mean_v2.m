...
TAdistA=linspace(1,120,200); % set TA dist to go over
...
keyboard
%}
[rmsCir,rmsMH,rmsGv2,rmsHGv2] = RMS_G_HG(numbSamp,sigmaA,TAdistA,coeM,mu,OrbType,rp);
rmsEc=rmsCir;
offSet=1;
%% pLots
close all;
figure(1)
surf(MAdistA(offSet:end),sigmaA(2:end),rmsCir(2:end,offSet:end));
ylabel('\sigma')
xlabel('$\bigtriangleup$ M (deg)','Interpreter','latex')
zlabel('RMS - a ')
set(gca,'zscale','log')
tiS=sprintf('Gibbs Method RMS-a ');
title(tiS)
colorbar

figure(2)
surf(MAdistA(offSet:end),sigmaA(2:end),rmsMH(2:end,offSet:end))
ylabel('\sigma')
xlabel('$\bigtriangleup$ M (deg)','Interpreter','latex')
zlabel('RMS-a')
set(gca,'zscale','log')
tiS=sprintf('Herrick Gibbs RMS-a');
title(tiS)
colorbar
...
figure(3)
surf(MAdistA(offSet:end),sigmaA(2:end),rmsBest(2:end,offSet:end));
ylabel('\sigma')
xlabel('$\bigtriangleup$ M (deg)','Interpreter','latex')
zlabel('RMS-a')
set(gca,'zscale','log')
tiS=sprintf('Best RMS');
title(tiS)
colorbar
...
function [rmsM,rmsMH,rmsGv2,rmsHGv2] = RMS_G_HG(numbSamp,sigmaA,TAdistA,coeM,mu,OrbType,rp)
    if strcmp(OrbType,'circ')%OrbType=='circ'
        fgid=101;
    else
        fgid=99;
